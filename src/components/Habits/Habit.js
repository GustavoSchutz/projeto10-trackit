    ...
    return (
        <div className="habits">
            {habitsList.map((habit) => {
                return <div key={habit.id} className="habit"><p>{habit.name}</p><div className="week">{isInside(habit.days).map((day) => {
                    
                    return <div key={day} className="day selected" ><p>{daysArr[1].day}</p></div>
                })}</div></div>
            })}
        </div>
